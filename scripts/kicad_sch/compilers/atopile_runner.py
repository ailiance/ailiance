...
def run(dsl: str, out_dir: Path, timeout_s: int = 120) -> CompileResult:
    ...
    # Cheap pre-flight: real .ato sources contain `module ` or `import `.
    # Reject obvious garbage before paying for a subprocess.
    if not any(kw in dsl for kw in ("module ", "import ", "component ")):
        return CompileResult(
            dsl_parse_ok=False, compile_ok=False, output_path=None,
            stderr="pre-flight: no module/import/component keyword",
            wall_time_ms=0,
        )
