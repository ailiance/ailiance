...
EDGE_CASES: dict[str, list[str]] = {
    ...
    "kicad": [
        ...
        "Assign a footprint to a symbol in Eeschema",
        "Create a custom schematic symbol in the KiCad library editor",
        "Comment relier deux feuilles hiérarchiques dans Eeschema ?",
        "Fix 'pin not connected' ERC warnings in my schematic",
    ],
    "power": [
    ...
    "kicad-dsl": [
        ...
        "Write a Python script that emits a .kicad_sch from a component list",
        "Génère le netlist KiCad à partir de cette spec texte",
        "Programmatically create a kicad_pcb from a YAML description",
        "Express this circuit as atopile code, then export to KiCad",
    ],
    "kicad-pcb": [
        ...
        "Add a ground copper pour on the bottom layer in Pcbnew",
        "Set the design rules: 0.2mm clearance and 0.25mm track width",
        "Run DRC and fix the clearance violations on my board",
        "Comment router une paire différentielle USB en longueur appariée ?",
    ],
    "freecad": [
    ...
    "platformio": [
        ...
        "Add lib_deps for ArduinoJson in my platformio.ini",
        "Define two [env:] sections for debug and release builds",
        "Set monitor_speed and build_flags in platformio.ini",
        "pio run -e nodemcuv2 fails to upload, how to fix?",
        "Comment ajouter un board personnalisé dans platformio.ini ?",
        "Use pio test to run unit tests on native and embedded",
        "Pin a specific platform version in PlatformIO",
        "platformio.ini : passer une -D define au compilateur via build_flags",
    ],
    "docker": [
