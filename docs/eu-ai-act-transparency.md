...
**Document ID:** AILIANCE-TRANS-001
**Date:** 2026-05-06
**Version:** 0.5.0
...
AILIANCE is a multi-model routing system that dispatches user queries to one of three EU-origin language models, each enhanced with domain-specific LoRA adapters. The system runs locally on a single machine (no cloud dependencies).
...
| **Intended use** | Routing fallback / quick-response worker when the MiniLM domain classifier cannot confidently match a labelled domain. Also surfaced as a chat-eligible card on `https://ailiance.fr`. |
...
- [x] JWT secret management — DONE (moved from hardcoded to env var `AILIANCE_JWT_SECRET`)
...
3. ailiance Devstral adapters slightly **degrade** HumanEval+ (style
